...
- Current package version at the time of this handoff: `0.1.14`
- Last target release for this handoff update: `v0.1.14`
- Windows installer output is produced by `npm run package:win` and normalized by GitHub Actions as `LocalLeaf-Host-Setup.exe`.
- GitHub release assets expected for `v0.1.14`:
...
- Editor top bar keeps primary actions visible and moves secondary actions into a hamburger-style expandable menu.
...
- Connected guests can download the project ZIP while the session is live. Guest tokens are revoked when the host stops the session.
...
The `0.1.14` hardening and final-MVP audit pass added:

- A private host token generated by `createLocalLeafServer()` and opened by the Electron shell as `?host=...`.
- Host/admin APIs, host WebSocket identity, SSE host state, asset/PDF links, and downloads now require the host token instead of trusting a localhost-looking `Host` header.
- Guest tokens are valid only while a session is live and are cleared on session stop.
- Hosts no longer get sent to the guest-style Session Ended screen when they stop hosting; the app returns to Home and can continue local editing.
- Static/API/SSE/download responses include security headers, including `Referrer-Policy: no-referrer`, `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, and a restrictive CSP compatible with the current UI.
- ZIP import now enforces maximum entry count, expanded size, per-file size, and nesting depth to reduce disk/RAM exhaustion risk.
- ZIP export skips dotfiles and `node_modules`, matching what the LocalLeaf file tree exposes.
- Project symlinks are skipped in the tree and rejected when resolving project paths.
- Project `latexmkrc` is ignored by default for safer local compilation. Only set `LOCALLEAF_ALLOW_LATEXMKRC=1` for fully trusted projects.
- Compile success now respects compiler exit status; if a PDF is produced but LaTeX reports errors, LocalLeaf marks the compile as failed while keeping the PDF visible for inspection.
- Old temp compile output directories are cleaned up when a newer successful PDF replaces them or when a failed output dir is no longer needed.
- The editor top row is simplified: Back, Save, title/status, Recompile, Export, and a hamburger-style More menu.
- The More menu contains Check for updates, Set main, Download ZIP, and toggles for Files, Editor, PDF, Logs, and Chat/Users.
- Mobile/tablet editor restore controls are more reachable, and the file/chat panels behave like overlay drawers on narrow screens.
- Validation for this pass covered `node --check`, `npm test`, `npm audit --audit-level=moderate`, and `git diff --check`.
